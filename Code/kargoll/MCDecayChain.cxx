#include "MCDecayChain.h"
#include "TLorentzVector.h"
#include <cstdlib>
#include "HistoConfig.h"
#include <iostream>

MCDecayChain::MCDecayChain(TString Name_, TString id_):
  Selection(Name_,id_)
{
}
...
MCDecayChain::~MCDecayChain(){
  for(unsigned int j=0; j<Npassed.size(); j++){
    std::cout << "MCDecayChain::~MCDecayChain Selection Summary before: "
	 << Npassed.at(j).GetBinContent(1)     << " +/- " << Npassed.at(j).GetBinError(1)     << " after: "
	 << Npassed.at(j).GetBinContent(NCuts+1) << " +/- " << Npassed.at(j).GetBinError(NCuts) << std::endl;
  }
  std::cout << "MCDecayChain::~MCDecayChain()" << std::endl;
}

void  MCDecayChain::Configure(){
  // Setup Cut Values
